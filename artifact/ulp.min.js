...
if (window.formType && window.siteId) {
    // @ts-ignore
    const site = (0, _brandForm.getWixSite)(window.siteId);
    const brandTitle = (0, _brandForm.getBrandTitleFn)(document.getElementsByTagName("p") ?? {}, // @ts-ignore
    (0, _brandForm.getDefaultPageTitle)(window.formType, window.siteId));
...
},{"./brandForm":"lqEUo","./constants":"iRfSK"}],"lqEUo":[function(require,module,exports) {
...
parcelHelpers.export(exports, "getSiteTitle", ()=>getSiteTitle);
...
const getSiteTitle = (formType, siteId)=>{
    console.info(`SSO Flow - ULP - site_id: ${siteId}`);
    console.info(`SSO Flow - ULP - form type: ${formType}`);
    const site = (0, _constants.WixSites)[siteId];
    if (!site) return formType === "login" ? (0, _constants.WixSites).sesam.loginSubTitle : (0, _constants.WixSites).sesam.signupSubTitle;
...
const LOGO_IMG_ID = "custom-prompt-logo";
const SiteIds = {
    hubspot: "HubSpot",
